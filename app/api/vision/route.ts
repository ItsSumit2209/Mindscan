...
async function callFaceCNN(arrayBuffer: ArrayBuffer, filename: string, mimeType: string) {
  ...
  try {
    ...
    const response = await fetch(`${mlServiceUrl}/analyze-face`, {
      method: 'POST',
      body: formData,
      signal: AbortSignal.timeout(10000),
    });
    ...
    console.error("Face CNN call failed:", err);
    return null;
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const [googleVision, faceCnn] = await Promise.all([
  callGoogleVision(base64Image),
  callFaceCNN(arrayBuffer, image.name, image.type),
]);
    ...
    if (clientEmotions && clientEmotions.face_detected) {
      ...
      if (googleSorrow > 0.4) { detected_face_emotion = "Sad"; face_score = 0.8 + googleSorrow * 0.15; }
          ...
          if (faceCnn && faceCnn.faces_detected > 0) {
        const cnnEmo = faceCnn.dominant_emotion;
        const cnnConf = faceCnn.confidence ?? 0;
        if ((cnnEmo === "Sad" || cnnEmo === "Fear") && detected_face_emotion !== "Sad" && cnnConf > 0.5) {
          face_score = Math.min(face_score + 0.1, 0.95);
        }
      }
